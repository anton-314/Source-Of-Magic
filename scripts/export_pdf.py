...
Compiles the manuscript into a PDF file using Pandoc + XeLaTeX.
Chapters are concatenated in order and passed to Pandoc with metadata.

Usage:
    python scripts/export_pdf.py [--output dist/source-of-magic.pdf]
...
DEFAULT_OUTPUT = ROOT / "dist" / "source-of-magic.pdf"
...
def main() -> None:
    ...
    print(f"PDF written to: {args.output.relative_to(ROOT)}")
